...
# при применении нормального ядра к нормализованным данным с построением графиков
...
h = [[0.03, 0.05], [0.03, 0.05], [0.03, 0.05], [0.03, 0.05], [0.05, 0.07]]  # h_bio, h_sim для каждого сенсора
...
#   print(len(sim_data[i]))
    ...
    density_estim_bio = density_estim(normal(bio_data[i]), h[i][0], values, 'normal')  # h[i-5][0], h[i-10][0]
    density_estim_neuron = density_estim(normal(sim_data[i]), h[i][1], values, 'normal')

#   print(density_estim_bio)
#   print(density_estim_neuron)

    T = statistics(normal(bio_data[i]), normal(sim_data[i]), h[i][0], h[i][1])
    print('h_bio ', h[i][0], 'h_sim ', h[i][1])
    print('значение статистики ', T)

    p = p_value(normal(bio_data[i]), normal(sim_data[i]), h[i][0], h[i][1])
    print('p-value ', p)
    print()

    # Отрисока графиков для 5 сенсоров
    plt.subplot(5, 1, i+1)  # i-5+1 i-10+1
    plt.plot(values, density_estim_bio, 'slategrey', label='bio. h_bio = %.2f, h_sim = %.2f' % (h[i][0], h[i][1]))
