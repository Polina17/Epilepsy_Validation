...
for j in range(15):
    result = []
    for i in range(2000):
        result.append(data[i, j, 5])  # № записи
        result.append(data[i, j, 40])
...
plt.subplot(3, 1, 1)
plt.title('Симуляция 2 / Эксперимент 23. Обобщённые нормализованные данные, cos ядро')
...
plt.subplot(3, 1, 2)
plt.title('Обобщённые нормализованные данные. Фильтрация [-0.1; 0.1]')
