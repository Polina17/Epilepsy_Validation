...
mat1 = scipy.io.loadmat('/home/polina/диплом/эпилепсия_данные_био/2011 may 03 P32 BCX rust/2011_05_03_0010.mat', squeeze_me=True)
data1 = mat1['lfp']
print(data1.shape)

bio_d = bio(data, 27)
bio_data = bio(data1, 41)
...
h = [15, 15]  # h_bio, h_bio_1

for i in range(0, 15):  # 0,5  5,10  10,15
    bio_data_sort = sorted(bio_d[i])  # Сортируем нормализованные данные
    bio_data_sort1 = sorted(bio_data[i])

    print(bio_data_sort)
    print(bio_data_sort1)

    bio_test = []
    for elem in bio_data_sort:
        if elem > 400 or elem < -400:  # Фильтруем
            bio_test.append(elem)

    bio_test1 = []
    for elem in bio_data_sort1:
        if elem > 400 or elem < -400:  # Фильтруем
            bio_test1.append(elem)

    print()

    min_v = min(min(bio_test), min(bio_test1))
    max_v = max(max(bio_test), max(bio_test1))

    print(min_v, max_v)

    values = []  # формируем массив значений, в которых будем искать плотность
    step = (max_v - min_v) / 5000  # число зависит от объёма выборки
    for j in range(5001):
        value = min_v + (step * j)
    ...
    density_estim_bio.append(density_estim(bio_test, h[0], values, 'cos'))  # h[i-5][0]
    density_estim_bio1.append(density_estim(bio_test1, h[1], values, 'cos'))  #h[i-5][1]

#   print(density_estim_bio)
#   print(density_estim_neuron)
    print('Сенсор ', i+1)
    T.append(statistics(bio_test, bio_test1, h[0], h[1]))
    print('h_bio ', h[0], 'h_bio1 ', h[1])
    print('значение статистики ', T[i])

    p.append(p_value(bio_test, bio_test1, h[0], h[1]))
    print('p-value ', p[i])
    print()
...
plt.suptitle('Эксперимент 3. Запись 27 / Эксперимент 10. Запись 41. Фильтрация [-400; 400]. h_bio = %.1f, h_bio_1 = %.1f' % (h[0], h[1]))
ax[0, 0].set_title('Cенсоры 1-8')
ax[0, 0].plot(values_array[0], density_estim_bio[0], 'g', label='bio', linewidth=0.8)
ax[0, 0].plot(values_array[0], density_estim_bio1[0], 'b', label='bio_1', linewidth=0.8)
ax[1, 0].plot(values_array[1], density_estim_bio[1], 'g', label='bio', linewidth=0.8)
ax[1, 0].plot(values_array[1], density_estim_bio1[1], 'b', label='bio_1', linewidth=0.8)
ax[2, 0].plot(values_array[2], density_estim_bio[2], 'g', label='bio', linewidth=0.8)
ax[2, 0].plot(values_array[2], density_estim_bio1[2], 'b', label='bio_1', linewidth=0.8)
ax[3, 0].plot(values_array[3], density_estim_bio[3], 'g', label='bio', linewidth=0.8)
ax[3, 0].plot(values_array[3], density_estim_bio1[3], 'b', label='bio_1', linewidth=0.8)
ax[4, 0].plot(values_array[4], density_estim_bio[4], 'g', label='bio', linewidth=0.8)
ax[4, 0].plot(values_array[4], density_estim_bio1[4], 'b', label='bio_1', linewidth=0.8)
ax[4, 0].set_ylabel('Значение плотности вероятности')
ax[5, 0].plot(values_array[5], density_estim_bio[5], 'g', label='bio', linewidth=0.8)
ax[5, 0].plot(values_array[5], density_estim_bio1[5], 'b', label='bio_1', linewidth=0.8)
ax[6, 0].plot(values_array[6], density_estim_bio[6], 'g', label='bio', linewidth=0.8)
ax[6, 0].plot(values_array[6], density_estim_bio1[6], 'b', label='bio_1', linewidth=0.8)
ax[7, 0].plot(values_array[7], density_estim_bio[7], 'g', label='bio', linewidth=0.8)
ax[7, 0].plot(values_array[7], density_estim_bio1[7], 'b', label='bio_1', linewidth=0.8)
ax[7, 0].set_xlabel('Потенциал локального поля, мВ')
ax[0, 1].set_title('Cенсоры 9-15')
ax[0, 1].plot(values_array[8], density_estim_bio[8], 'g', label='bio', linewidth=0.8)
ax[0, 1].plot(values_array[8], density_estim_bio1[8], 'b', label='bio_1', linewidth=0.8)
ax[1, 1].plot(values_array[9], density_estim_bio[9], 'g', label='bio', linewidth=0.8)
ax[1, 1].plot(values_array[9], density_estim_bio1[9], 'b', label='bio_1', linewidth=0.8)
ax[2, 1].plot(values_array[10], density_estim_bio[10], 'g', label='bio', linewidth=0.8)
ax[2, 1].plot(values_array[10], density_estim_bio1[10], 'b', label='bio_1', linewidth=0.8)
ax[3, 1].plot(values_array[11], density_estim_bio[11], 'g', label='bio', linewidth=0.8)
ax[3, 1].plot(values_array[11], density_estim_bio1[11], 'b', label='bio_1', linewidth=0.8)
ax[4, 1].plot(values_array[12], density_estim_bio[12], 'g', label='bio', linewidth=0.8)
ax[4, 1].plot(values_array[12], density_estim_bio1[12], 'b', label='bio_1', linewidth=0.8)
ax[5, 1].plot(values_array[13], density_estim_bio[13], 'g', label='bio', linewidth=0.8)
ax[5, 1].plot(values_array[13], density_estim_bio1[13], 'b', label='bio_1', linewidth=0.8)
ax[6, 1].plot(values_array[14], density_estim_bio[14], 'g', label='bio', linewidth=0.8)
ax[6, 1].plot(values_array[14], density_estim_bio1[14], 'b', label='bio_1',linewidth=0.8)
